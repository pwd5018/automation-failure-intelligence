...
Phase 7 is the UI redesign and triage workspace. The current slices establish a responsive shell with a run sidebar, results workspace, persistent desktop inspector, center-panel tabs, an accessible responsive ingestion drawer, Jira-inspired visual hierarchy, and keyboard/copy polish for investigation workflows while preserving the existing API and source-result semantics. The remaining redesign will introduce React with Vite as the frontend foundation while keeping Express as the API/server owner, preserving `/api/*`, `/developer.html`, uploads, PostgreSQL persistence, and the existing source-result contract. Manual ambiguity resolution is deferred to Phase 8: users will be able to split or combine raw `NEEDS REVIEW` records for one run without changing the source XML. Phase 9 covers external CI/webhook ingestion, authentication, collaboration, and real Jira integration. Normalized testcase and failure-group occurrence storage is active for PostgreSQL, while the API preserves its existing JSONB-compatible response contract. Retry policy configuration is persisted per project, but source metadata remains the authority for retry interpretation. Current framework adapters identify only explicitly declared report metadata; they do not infer framework behavior.
