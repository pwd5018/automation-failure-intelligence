...
Manual ambiguity resolution is the next Phase 7 slice: users will be able to split or combine raw `NEEDS REVIEW` records for one run without changing the source XML. Phase 8 covers external CI/webhook ingestion, authentication, collaboration, and real Jira integration. Normalized testcase and failure-group occurrence storage is active for PostgreSQL, while the API preserves its existing JSONB-compatible response contract. Retry policy configuration is persisted per project, but source metadata remains the authority for retry interpretation. Current framework adapters identify only explicitly declared report metadata; they do not infer framework behavior.
