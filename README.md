...
Phase 7 is the implemented UI redesign and triage workspace. Phase 8 is the next step: a repository-local fake app and TestNG producer suite will generate master and individual JUnit XML files so bundle ingestion can be designed against realistic output. The producer is development/test infrastructure and will not be part of the Vercel runtime. Manual ambiguity resolution is deferred to Phase 9: users will be able to split or combine raw `NEEDS REVIEW` records for one run without changing the source XML. Phase 10 covers external CI/webhook ingestion, authentication, collaboration, and real Jira integration. Normalized testcase and failure-group occurrence storage is active for PostgreSQL, while the API preserves its existing JSONB-compatible response contract. Retry policy configuration is persisted per project, but source metadata remains the authority for retry interpretation. Current framework adapters identify only explicitly declared report metadata; they do not infer framework behavior.
